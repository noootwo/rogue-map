...
async function benchmark() {
  ...
  // --- Native Map ---
  {
    ...
    console.log(
      `Heap Used: ${formatBytes(memAfter.heapUsed - startMem.heapUsed)}`,
    );
  ...
  // --- RogueMap (Optimized) ---
  {
    ...
    console.log(
      `Heap Used: ${formatBytes(memAfter.heapUsed - midMem.heapUsed)}`,
    );
    console.log(
      `External/ArrayBuffer: ${formatBytes(memAfter.external + memAfter.arrayBuffers - (midMem.external + midMem.arrayBuffers))}`,
    );
    console.log(
      `Total Memory Delta: ${formatBytes(memAfter.rss - midMem.rss)}`,
    );
