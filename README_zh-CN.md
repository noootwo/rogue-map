...
- **堆外存储 (Off-Heap)**: 使用单个大 `Buffer` 存储数据，显著降低 V8 垃圾回收 (GC) 压力和堆内存使用。
- **高效内存**: 在存储大量数据（百万级）时，内存占用比原生 `Map` 少约 50%~99%。
- **高性能**:
  - 极快的写入速度（线性探测 + 追加式日志）。
  - 针对字符串优化的读取性能（直接缓冲区比较）。
...
| --- | --- | --- | --- |
| **写入耗时** | ~258ms | **~204ms** | **快 21%** 🚀 |
| **读取耗时** | ~18ms | ~251ms | 慢 14x (需解码) |
| **堆内存 (Heap)** | ~58 MB | **~0.1 MB** | **减少 99%** 📉 |
| **总内存 (RSS)** | ~93 MB | **~37 MB** | **减少 61%** |
...
| 指标 | 原生 Map (V8) | RogueMap | 差异 |
| --- | --- | --- | --- |
| **写入耗时** | ~261ms | ~550ms | 慢 2x (JSON 序列化开销) |
| **堆内存 (Heap)** | ~104 MB | **~0.1 MB** | **减少 99%** |

> **核心结论**: 对于标量数据（数字、短字符串），RogueMap 显著更快且更轻量。对于复杂对象，RogueMap 以 CPU（序列化）换取了巨大的内存节省和堆外存储能力。
...
RogueMap 使用基于单个 `Buffer` 的 **线性探测哈希表 (Linear Probing Hash Table)**。

- **Buckets**: `Int32Array`，存储指向数据 Buffer 的偏移量。
